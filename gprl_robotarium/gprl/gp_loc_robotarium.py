...
def calculate_local_rssi(robot):
    ...
    rssi_co_var = np.zeros((int(local_grid_size[1]/resolution)+2, int(local_grid_size[0]/resolution)+2))
    for j, y_pos in enumerate(y_local_values[robot]):
        for i, x_pos in enumerate(x_local_values[robot]):
            rssi_values[j][i],rssi_co_var[j][i]=robot_gp_models[robot].predict(np.array([[x_pos, y_pos]]))
    ...
    return rssi_values,rssi_co_var

def calculate_local_res_rssi(ap_pos,res,robot):
    x_values = np.arange(ap_pos[0]-res*5,ap_pos[0]+res*5,res)
    y_values = np.arange(ap_pos[1]-res*5,ap_pos[1]+res*5,res)
    rssi_values = np.zeros((x_values.size,y_values.size))
    rssi_co_var = np.zeros((x_values.size,y_values.size))
    for j, y_pos in enumerate(y_values):
        for i, x_pos in enumerate(x_values):
            rssi_values[j][i],rssi_co_var[j][i]=robot_gp_models[robot].predict(np.array([[x_pos, y_pos]]))
            # rssi_values[j][i]=gen_wifi(size=(x_values.size,y_values.size),pos=((x_pos*10),(y_pos*10)),grid_pos=(j,i),source_position=source_positions["robot"+str(robot+1)])
    return rssi_values,rssi_co_var
...
for i, ax in enumerate(ax_local):
    ...
    z_values,_=calculate_local_rssi(i)
    z_values = z_values[:,:-1]
...
co_variances = [[] for _ in range(N)]

f = open('error_data.csv', 'w')

# create the csv writer
error_writer = csv.writer(f)
error_writer.writerow(['iteration','robot1_rmse','robot1_std','robot2_rmse','robot2_std','robot3_rmse','robot3_std'])
...
while index<iterations:
    ...
    while index<iterations and np.any(np.linalg.norm(x[:2, :] - goal_points[:2, :], axis=0) > tolerance):
        ...
        for k in range(5):
            for i in range(N):
                if k == 0:
                    rssi_values,rssi_co_var = calculate_local_rssi(i)
                    rssi_vals.append(rssi_values)
                    ap_pos = unravel_index(rssi_values[:,:-1].argmax(), rssi_values[:,:-1].shape)
                    # print(ap_pos)
                    ...
                    res = res/2
                    rssi_values,rssi_co_var = calculate_local_res_rssi(AP_positions[:,i],res,i)
                    ap_pos = unravel_index(rssi_values.argmax(), rssi_values.shape)
                    # print(ap_pos)
                    AP_positions[0][i]=(ap_pos[1]*res)+(AP_positions[0][i]-(res*5))
                    AP_positions[1][i]=(ap_pos[0]*res)+(AP_positions[1][i]-(res*5))
                    if k == 4:
                        co_variances[i].append(rssi_co_var[ap_pos[0],ap_pos[1]])
        ...
        error_writer.writerow([index,localization_errors[0][index],localization_error_std[0][index],localization_errors[1][index],localization_error_std[1][index],localization_errors[2][index],localization_error_std[2][index]])
       ...
       # Update the error plots for each robot
        ...
        #     std_values = np.array(localization_error_std[i])
            ...
            # ax_error[i].plot(x_values, np.array(co_variances[i]), color='black', linewidth=2)
